<template>
  <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-700 px-4 py-3 text-lg text-slate-300">
    ...
    <div class="rounded-lg border border-slate-600 p-1 text-sm">
      <button class="rounded px-3 py-1" :class="view === 'list' ? 'bg-slate-700 text-slate-200' : 'text-slate-400'" type="button" @click="$emit('update:view', 'list')">☰</button>
      <button class="rounded px-3 py-1" :class="view === 'grid' ? 'bg-slate-700 text-slate-200' : 'text-slate-400'" type="button" @click="$emit('update:view', 'grid')">◻</button>
    </div>
  </div>
</template>
